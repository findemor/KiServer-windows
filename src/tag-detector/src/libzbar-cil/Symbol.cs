...
namespace ZBar
{
	...
	public class Symbol
	{
		...
		internal Symbol(IntPtr symbol){
            ...
            long polygonSize = zbar_symbol_get_loc_size(symbol);
            Point[] polygon = new Point[polygonSize];
            for(int i = 0; i < polygonSize; i++)
            {
                int x = zbar_symbol_get_loc_x(symbol, Convert.ToUInt32(i));
                int y = zbar_symbol_get_loc_y(symbol, Convert.ToUInt32(i));
